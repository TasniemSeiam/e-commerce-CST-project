...
if (filterText) {
  ...
  if (filteredProducts.length > 0) {
    ...
    wrapperDiv.innerHTML = '<h2 class=" w-100 text-center p-5 my-3 ">No products found in this category </h2>';
  }
} else {
  const products = JSON.parse(localStorage.getItem('products')) ||[];
...
searchBTN.addEventListener('click', (e) => {
  ...
  if (searchValue) {
    ...
    if (filteredProducts.length > 0) {
      ...
      wrapperDiv.innerHTML = '<h2 class=" w-100 text-center p-5 my-3 bg-white" >No products found matching your search </h2>';
    }
  }
});
// })/////
...
function displayProduct(product, _location) {
  product.forEach((product) => {
    ...
    wishlistBtn.className = "mx-auto mx-md-0  wishlistHeart btn-white  rounded-circle fas fa-heart wishlistHearti";
